  ...
  <div class="boxes">
    ...
    <div v-for="post in allItems" :key="post.id" class="row">
      <div class="container" v-if="!isLoading">
        ...
        <p style="color: white">{{post.latestMsg.messageText}}</p>
        <span class="time-right">{{post.latestMsg.timestamp.toDate()}}</span>
      </div>
        ...
        for (let user of channel.recipients) {
          if (user.first != this.userProfile.uid) {
            this.fetchUser(user.first).then(data => {
              console.log(data);
              this.listOfUser.push({ user: data, id: channel.id });
            });
          }
        }
      }
